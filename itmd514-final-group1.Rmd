...
# Split Dataset
lowPrice <- subset(housedata, price <= priceThreshold)
highPrice <- subset(housedata, price > priceThreshold)

# Overlaid Histogram
ggplot() +
  geom_histogram(data = lowPrice, aes(x = sqft_above, fill = "Low"), binwidth = 100) +
  geom_histogram(data = highPrice, aes(x = sqft_above, fill = "High"), binwidth = 100) +
  scale_fill_manual(name = "PriceType", values = c("red", "blue")) +
  labs(title = "Overlay Histogram of House Area and Price",x = "Square Footage", y = "Frequency") +
  theme_classic()
```
